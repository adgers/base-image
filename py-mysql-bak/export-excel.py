...
products = "SELECT DATE_ADD(t_sku.f_created_time, INTERVAL 8 HOUR) AS 创建时间, t_sku.f_display_code AS 商品sku号,t_sku.f_name AS 商品sku名称, t_product.f_display_code AS 商品spu号, t_product.f_name AS 商品名, GROUP_CONCAT(CONCAT(t_sku_attribute.f_name,':','  ',t_sku_attribute.f_value,' ')) AS 商品属性, t_product.f_genre AS 商品类型, t_product.f_unit AS 计量单位, t_category.f_name AS 分类名称, t_brand.f_name AS 品牌名称, t_supplier.f_name AS 供应商名称, t_sku.f_international_code AS 外部编码, t_sku_price.f_value AS 零售价, CASE t_sku.f_status WHEN 1 THEN '上架' WHEN 2 THEN '下架' ELSE '状态未知' END AS 销售状态 FROM commodity.t_sku JOIN commodity.t_product ON t_sku.f_product_id = t_product.f_id JOIN commodity.t_category ON t_product.f_category_id = t_category.f_id JOIN commodity.t_supplier ON t_product.f_supplier_id = t_supplier.f_id JOIN commodity.t_brand ON t_product.f_brand_id = t_brand.f_id JOIN commodity.t_sku_price ON t_sku.f_display_code = t_sku_price.f_sku_code  JOIN commodity.t_sku_attribute ON t_sku_attribute.f_sku_code = t_sku.f_display_code WHERE t_sku_price.f_config_id = 1 GROUP BY 商品sku号 ORDER BY 创建时间 DESC;"
